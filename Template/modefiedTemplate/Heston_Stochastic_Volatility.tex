...
The contribution of this paper is threefold. First, we implement the Quadratic Exponential (QE) scheme to simulate the volatility process in Guaranteed Automatic Integration Library (GAIL) so that we get the simulation algorithm of Heston model with guaranteed automatically realized user-defined error tolerance. Second, when setting the volatility of the asset prices' volatility as zero, the Broadie-Kaya scheme, which is used to discretize the asset price process, gives inaccurate results. We use a change of variables to modify this problem. We compare the results with other simulation methods such as geometric Brownian motion and quasi-Monte Carlo, when setting the volatility of asset prices' volatility as zero, because in this case, the algorithm works like the algorithm of asset price process with deterministic volatility. It shows that the new algorithm is accurate and fast. Third, we apply variance reduction techniques such as importance sampling, control variates and Quasi-Monte Carlo methods to increase accuracy. The advantages of the new algorithm is strengthened.

There are several well-known stochastic volatility models: the Hull-white model (1987), the Scott-Chesny model (1989), the Heston model (1993) and the SABR model (2002). The Heston model is of particular interest to us since it is one of the most widely used stochastic volatility models.
