...
\frac{dY_t}{\theta-Y_t} &=\kappa dt\\
-ln(\theta-Y_t) &=\kappa t+C_1\\
Y_t &=\theta+Ce^{-\kappa t}\\
\end{align*}

\begin{align*}
   \int^{t+\Delta}_t Y_udu & =\theta\Delta+C\int^{t+\Delta}_t e^{-\kappa u}du \\
    & =\theta\Delta+\frac{Ce^{-\kappa t}-Ce^{-\kappa (t+\Delta)}}{\kappa}\\
    &= \theta\Delta+\frac{V_t-V_{t+\Delta}}{\kappa}
  \end{align*}
$\theta=\frac{V(t)-e^{\kappa\Delta}V(t+\Delta)}{1-e^{\kappa\Delta}}$, then
\begin{equation*}
 \int_{t}^{t+\Delta}(V_u-U_u)\d u =\int^{t+\Delta}_t Y_udu= \Delta(\gamma_1Y_t+(1-\gamma_1)Y_{t+\Delta})
\end{equation*}
\begin{equation*}
\begin{split}
...
\end{align*}
\begin{align*}
  \int_{t}^{t+\Delta}U_u\d u & = \int_{t}^{t+\Delta}U_u\d (u+A) \\
   & = U_u(u+A)|_t^{t+\Delta}-\int_{t}^{t+\Delta}(u+A)\d U_u \\
   & = U_u(u+A)|_t^{t+\Delta}-\int_{t}^{t+\Delta}(u+A)(-\kappa U_u\d u +\nu\sqrt{V_u}\d W_u)
\end{align*}
Let $A=-t-\Delta\gamma_1$,
\begin{align*}
  \int_{t}^{t+\Delta}U_u\d u & = U_{t+\Delta}(t+\Delta-t-\Delta\gamma_1)-U_{t}(-\Delta\gamma_1)-\int_{t}^{t+\Delta}(u-t-\Delta\gamma_1)(-\kappa U_u)\d u\\
  & -\int_{t}^{t+\Delta}(u-t-\Delta\gamma_1)\nu\sqrt{V_u}\d W_u \\
   & = \Delta(\gamma_1U_t+(1-\gamma_1) U_{t+\Delta})-\int_{t}^{t+\Delta}(u-t-\Delta\gamma_1)(-\kappa U_u)\d u-\int_{t}^{t+\Delta}(u-t-\Delta\gamma_1)\nu\sqrt{V_u}\d W_u
\end{align*}
\begin{align*}
   \int_{t}^{t+\Delta}(u-t-\Delta\gamma_1)(-\kappa U_u)\d u & = \int_{t}^{t+\Delta}\frac{1}{2}(-\kappa U_u)\d (u-\Delta\gamma_1)^2\\
   & = -\int_{t}^{t+\Delta}\kappa U_u\d \big(\frac{(u-t-\Delta\gamma_1)^2}{2}+B\big)\\
   & = -\big(\kappa U_{u}\big(\frac{(u-t-\Delta\gamma_1)^2}{2}+B\big)\big|_t^{t+\Delta}\big)+\int_{t}^{t+\Delta}\big(\frac{(u-t-\Delta\gamma_1)^2}{2}+B\big)\kappa \d U_u\\
   & = -\kappa U_{t+\Delta}\big(\frac{\Delta^2(1-\gamma_1)^2}{2}+B\big)+\kappa U_{t}\big(\frac{\Delta^2\gamma_1^2}{2}+B\big)\\
   & + \int_{t}^{t+\Delta}\big(\frac{(u-t-\Delta\gamma_1)^2}{2}+B\big)\kappa\d U_u
\end{align*}
Let $B=-\frac{\Delta^2\gamma_1^2}{2}$,
\begin{align*}
   \int_{t}^{t+\Delta}(u-t-\Delta\gamma_1)(-\kappa U_u)\d u & = \kappa U_{t+\Delta}\big(\frac{(1-\gamma_1)^2-\gamma_1^2}{2}\Delta^2\big)+ \int_{t}^{t+\Delta}\big(\frac{(u-t-\Delta\gamma_1)^2}{2}-\frac{\Delta^2\gamma_1^2}{2}\big)\kappa \d U_u\\
   ...
   & = O(\nu\Delta^2)+\Delta\gamma_1\kappa^2\int_{t}^{t+\Delta}(u-t)U_u\d u\\
   \text{by Holder Inequality} &\leq O(\nu\Delta^2)+\Delta\gamma_1\kappa^2\sqrt{\int_{t}^{t+\Delta}(u-t)^2\d u\int_{t}^{t+\Delta}U_u^2\d u}\\
   & = O(\nu\Delta^2)
\end{align*}
Now we want to show Ito integral $I =\int_{t}^{t+\Delta}\big(\frac{(u-t)^2}{2}-(u-t)\Delta\gamma_1\big)\kappa\nu\sqrt{V_u}\d W_u$ is of order $O(\nu\Delta^2)$. By Theorem 4.7 of \cite{Klebaner2005}, $I$ is a continuous zero mean square integrable martingale.
\begin{align*}
 \mathbb{E}(I)&= 0\\
  ...
  &=\kappa^2\nu^2\bigg[\int_{t}^{(t+\Delta)^5}\frac{1}{20} (V_0 e^{-\kappa (u-t)}e^{-\kappa t}+\theta(1-e^{-\kappa (u-t)}e^{-\kappa t})) \d (u-t)^5\\
  &-\int_{t}^{(t+\Delta)^4}\frac{1}{4}\Delta\gamma_1 (V_0 e^{-\kappa (u-t)}e^{-\kappa t}+\theta(1-e^{-\kappa (u-t)}e^{-\kappa t}))\d (u-t)^4\\
  &+\int_{t}^{(t+\Delta)^3}\frac{\Delta^2}{3}\gamma_1^2(V_0 e^{-\kappa (u-t)}e^{-\kappa t}+\theta(1-e^{-\kappa (u-t)}e^{-\kappa t})) \d(u-t)^3\bigg]\\
  &=\kappa^2\nu^2 \Delta^5(V_0 e^{-\kappa \xi}+\theta(1-e^{-\kappa \xi}))(\frac{1}{20}-\frac{1}{4}\gamma_1+\frac{1}{3}\gamma_1^2)
\end{align*}
